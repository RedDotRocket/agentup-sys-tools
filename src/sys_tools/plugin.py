...
class Plugin:
    ...
    def can_handle_task(self, context: SkillContext) -> float:
        ...
        keywords = {
            # File operations
            'read file': 1.0, 'read': 0.8, 'open file': 1.0, 'view file': 1.0,
            'write file': 1.0, 'write': 0.8, 'save file': 1.0, 'create file': 1.0,
            'file exists': 1.0, 'check file': 0.9, 'file info': 1.0,
            'delete file': 1.0, 'remove file': 1.0,

            # Directory operations
            'list directory': 1.0, 'list files': 1.0, 'ls': 0.9, 'dir': 0.9,
            'create directory': 1.0, 'mkdir': 1.0, 'make directory': 1.0,
            'folder': 0.8, 'directory': 0.8,

            # System operations
            'system info': 1.0, 'system information': 1.0, 'platform': 0.9,
            'working directory': 1.0, 'pwd': 1.0, 'current directory': 1.0,
            'execute': 0.9, 'run command': 1.0, 'shell': 0.8,

            # General file system
            'file system': 0.9, 'filesystem': 0.9, 'path': 0.7
        }
    ...
    # File Operations  
    async def _internal_read_file(self, path: str, encoding: str = 'utf-8') -> dict[str, Any]:
        ...
        try:
            ...
            if not file_path.is_file():
                return create_error_response(
                    ValueError(f"Path is not a file: {path}"),
                    "read_file"
                )
    ...
    async def _get_file_info(self, path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not file_path.exists():
                return create_error_response(
                    FileNotFoundError(f"Path not found: {path}"),
                    "get_file_info"
                )
    ...
    # Directory Operations
    async def _list_directory(self, path: str = ".", pattern: str | None = None,
        ...
        try:
            ...
            if not dir_path.exists():
                return create_error_response(
                    FileNotFoundError(f"Directory not found: {path}"),
                    "list_directory"
                )

            if not dir_path.is_dir():
                return create_error_response(
                    ValueError(f"Path is not a directory: {path}"),
                    "list_directory"
                )
    ...
    async def _create_directory(self, path: str, parents: bool = True,
    ...
    async def _write_file(self, path: str, content: str, encoding: str = 'utf-8', create_parents: bool = True, **kwargs) -> str:
    ...
    async def _write_file_internal(self, path: str, content: str, encoding: str = 'utf-8', create_parents: bool = True) -> dict[str, Any]:
    ...
    async def _list_directory_internal(self, path: str = ".", pattern: str | None = None, recursive: bool = False) -> dict[str, Any]:
    ...
    async def _create_directory_internal(self, path: str, parents: bool = True, exist_ok: bool = True) -> dict[str, Any]:
    ...
    async def _delete_file_internal(self, path: str, recursive: bool = False) -> dict[str, Any]:
    ...
    async def _execute_command_internal(self, command: str, timeout: int = 30) -> dict[str, Any]:
    ...
    async def _list_directory(self, path: str = ".", pattern: str | None = None, recursive: bool = False, **kwargs) -> str:
    ...
    async def _create_directory(self, path: str, parents: bool = True, exist_ok: bool = True, **kwargs) -> str:
    ...
    async def _ai_get_working_directory(self, task, context: SkillContext) -> SkillResult:
